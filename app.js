...
app.use(`${subdomain}`, express.static(`${__dirname}/public`), { extensions: ['html'] })); // tells the app to use a public folder and remove the html extension
...
Line.prototype.meter_check = function (meter) {
    ...
    if (adjusted_meter !== meter && !word_not_found && !too_) {
...
Poem.prototype.check = async function (poem) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        let meter = lines[i].meter_check(this.meter[i]);
...
let PetrarchanSonnet = new Poem(14, '0101010101', 'ABBAABBACDECDE');
let ShakespereanSonnet = new Poem(14, '0101010101', 'ABABCDCDEFEFGG');
let SpensereanSonnet = new Poem(14, '0101010101', 'ABABBCBCCDCDEE');
let Limerick = new Poem(5, ['0100100100', '00100100100', '001001', '01001', '00100100100'], 'AABBA');
...
function create_list(file, output) {
    ...
    for (let i of words) {
        ...
        if (arr[0].charAt(0).match(/[a-zA-Z]/)) {

            let stresses = arr[1].match(/\d+/g);
